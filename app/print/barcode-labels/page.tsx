    ...
    <main className="min-h-screen bg-stone-50 p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-700">Etiquetas de código de barras</div>
            <h1 className="mt-2 text-3xl font-black text-stone-950">{displayName}</h1>
            <p className="mt-2 text-sm text-stone-500">Elige un tamaño de etiqueta y luego imprime desde esta hoja dedicada.</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2 print:hidden">
          {LABEL_SIZES.map((candidate) => (
            ...
            >
              {candidate === 'small' ? 'Pequeño' : candidate === 'medium' ? 'Mediano' : 'Grande'}
            </Link>
          ))}
        </div>
