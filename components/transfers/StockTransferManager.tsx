...
export default function StockTransferManager({
  ...
  function addLine() {
    ...
    if (!toShopId) {
      setError('Selecciona primero una sucursal de destino.');
      return;
    }

    if (!sourceProduct || !destinationProduct) {
      setError('Selecciona los productos de origen y destino.');
      return;
    }

    if (!Number.isFinite(parsedQty) || parsedQty <= 0) {
      setError('La cantidad a transferir debe ser mayor que cero.');
      return;
    }

    if (parsedQty > sourceProduct.stockQty) {
      setError(`${sourceProduct.name} solo tiene ${sourceProduct.stockQty} unidad(es) disponibles.`);
      return;
    }
  ...
  async function createTransfer() {
    ...
    if (!lines.length) {
      setError('Añade al menos una línea de transferencia.');
      return;
    }
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo crear la transferencia de inventario.' }));
    setLoading(false);

    if (!response.ok) {
      setError(data.error ?? 'No se pudo crear la transferencia de inventario.');
      return;
    }
    ...
    setLines([]);
    setNotes('');
    setSuccess('Transferencia de inventario guardada y marcada como pendiente de envío.');
  }
  ...
  async function updateTransfer(transferId: string, action: 'SEND' | 'RECEIVE' | 'CANCEL') {
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo actualizar la transferencia de inventario.' }));
    setBusyTransferId(null);

    if (!response.ok) {
      setError(data.error ?? 'No se pudo actualizar la transferencia de inventario.');
      return;
    }

    replaceTransfer(data.stockTransfer);
    if (action === 'SEND') {
      setSuccess('Transferencia enviada y existencias de origen reducidas.');
    } else if (action === 'RECEIVE') {
      setSuccess('Transferencia recibida y existencias de destino aumentadas.');
    } else {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
      <Card>
        <div className="mb-4">
          <div className="text-lg font-black text-stone-900">Crear transferencia entre sucursales</div>
          <div className="text-sm text-stone-500">Sucursal de origen: {activeShopName}</div>
        </div>
        ...
        {otherShops.length ? (
          <div className="space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Input placeholder="Notas de la transferencia" value={notes} onChange={(event) => setNotes(event.target.value)} />
            </div>
            ...
            <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
              ...
              {selectedSourceProduct ? (
                <div className="mt-3 text-sm text-stone-600">
                  Disponible en {activeShopName}: <span className="font-semibold text-stone-900">{selectedSourceProduct.stockQty}</span> unidad(es)
                </div>
              ) : null}
            </div>
                  ...
                  <div key={`${line.fromProductId}-${line.toProductId}-${index}`} className="flex items-center justify-between gap-3 rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
                    <div>
                      ...
                      <div className="text-xs text-stone-500">
                        {line.qty} unidad(es) a {line.toProductName}
                      </div>
                    </div>
                    <Button type="button" variant="ghost" onClick={() => removeLine(index)}>Quitar</Button>
                  </div>
                ))
              ) : (
                <div className="text-sm text-stone-500">Aún no se han añadido líneas de transferencia.</div>
              )}
            </div>
            ...
            <Button onClick={createTransfer} disabled={!lines.length || loading}>
              {loading ? 'Guardando transferencia...' : 'Guardar borrador de transferencia'}
            </Button>
          </div>
        ) : (
          <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-5 text-sm text-stone-500">
            Necesitas acceso a más de una sucursal para crear transferencias de inventario.
          </div>
        )}
      </Card>
      ...
      <Card>
        <div className="mb-4">
          <div className="text-lg font-black text-stone-900">Historial de transferencias</div>
          <div className="text-sm text-stone-500">Solo la sucursal activa puede enviar sus borradores o recibir transferencias dirigidas a ella.</div>
        </div>
        ...
        <div className="space-y-4">
          {history.length ? (
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        <Badge tone={isSourceBranch ? 'amber' : 'blue'}>
                          {isSourceBranch ? 'Salida' : 'Entrada'}
                        </Badge>
                      </div>
                    </div>
                    <div className="text-right text-sm text-stone-600">
                      <div>Creada {dateTime(transfer.createdAt)}</div>
                      {transfer.sentAt ? <div>Enviada {dateTime(transfer.sentAt)}</div> : null}
                      {transfer.receivedAt ? <div>Recibida {dateTime(transfer.receivedAt)}</div> : null}
                    </div>
                  </div>

                  {transfer.notes ? <div className="mt-3 text-sm text-stone-600">Notas: {transfer.notes}</div> : null}
                  ...
                  <div className="mt-4 space-y-2 rounded-2xl bg-stone-50 p-4">
                    {transfer.items.map((item) => (
                      <div key={item.id} className="rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm text-stone-700">
                        ...
                        <div className="text-xs text-stone-500">
                          {item.qty} unidad(es) a {item.destinationProductNameSnapshot}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {isSourceBranch && transfer.status === 'DRAFT' ? (
                      <Button type="button" onClick={() => updateTransfer(transfer.id, 'SEND')} disabled={busyTransferId === transfer.id}>
                        {busyTransferId === transfer.id ? 'Enviando...' : 'Enviar transferencia'}
                      </Button>
                    ) : null}
                    {isSourceBranch && transfer.status === 'DRAFT' ? (
                    ...
                    {isDestinationBranch && transfer.status === 'IN_TRANSIT' ? (
                      <Button type="button" variant="secondary" onClick={() => updateTransfer(transfer.id, 'RECEIVE')} disabled={busyTransferId === transfer.id}>
                        {busyTransferId === transfer.id ? 'Recibiendo...' : 'Confirmar recepción'}
                      </Button>
                    ) : null}
                  </div>
                </div>
            ...
            <div className="text-sm text-stone-500">Aún no hay transferencias entre sucursales registradas.</div>
          )}
        </div>
      </Card>
    </div>
