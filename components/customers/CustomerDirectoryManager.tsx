...
export default function CustomerDirectoryManager({
  ...
  async function saveCustomer() {
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo guardar el cliente.' }));
    setSaving(false);
  ...
  async function postPayment() {
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo registrar el pago de cuenta por cobrar.' }));
    setPostingPayment(false);

    if (!response.ok) {
      setError(data.error ?? 'No se pudo registrar el pago de cuenta por cobrar.');
      return;
    }

    replaceCustomer(data.customer);
    setPaymentDraft(paymentDraftFromCustomer(data.customer));
    setSuccess('Pago de cuenta por cobrar registrado.');
  }

  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card><div className="text-sm text-stone-500">Al día</div><div className="mt-2 text-3xl font-black text-stone-900">{money(agingSummary.current, currencySymbol)}</div></Card>
        <Card><div className="text-sm text-stone-500">1-30 días</div><div className="mt-2 text-3xl font-black text-amber-700">{money(agingSummary.days_1_30, currencySymbol)}</div></Card>
        <Card><div className="text-sm text-stone-500">31-60 días</div><div className="mt-2 text-3xl font-black text-orange-700">{money(agingSummary.days_31_60, currencySymbol)}</div></Card>
        <Card><div className="text-sm text-stone-500">61+ días</div><div className="mt-2 text-3xl font-black text-red-700">{money(agingSummary.days_61_plus, currencySymbol)}</div></Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <Card>
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700">Directorio de clientes</div>
              <h2 className="mt-2 text-xl font-black text-stone-900">{selectedCustomer ? 'Editar cliente' : 'Nuevo cliente'}</h2>
            </div>
            <Button type="button" variant="secondary" onClick={startNewCustomer}>Nuevo</Button>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Input placeholder="Nombre" value={form.firstName} onChange={(event) => setForm((current) => ({ ...current, firstName: event.target.value }))} />
            <Input placeholder="Apellido" value={form.lastName} onChange={(event) => setForm((current) => ({ ...current, lastName: event.target.value }))} />
            <Input placeholder="Nombre de la empresa" value={form.businessName} onChange={(event) => setForm((current) => ({ ...current, businessName: event.target.value }))} />
            <Input placeholder="Persona de contacto" value={form.contactPerson} onChange={(event) => setForm((current) => ({ ...current, contactPerson: event.target.value }))} />
            <Input placeholder="Teléfono" value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} />
            <Input placeholder="Correo electrónico" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} />
            <Input placeholder="Identificación fiscal / NIF / RUT" value={form.taxId} onChange={(event) => setForm((current) => ({ ...current, taxId: event.target.value }))} />
            <Input placeholder="Dirección" value={form.address} onChange={(event) => setForm((current) => ({ ...current, address: event.target.value }))} />
          </div>

          <textarea className="mt-3 min-h-24 w-full rounded-2xl border border-stone-200 bg-white/88 px-4 py-3 text-sm text-stone-900 outline-none transition placeholder:text-stone-400 hover:border-stone-300 focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/10" placeholder="Notas" value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} />
          ...
          <div className="mt-4 flex gap-2">
            ...
            {selectedCustomer ? <Button type="button" variant="ghost" onClick={startNewCustomer}>Cancelar</Button> : null}
          </div>
        </Card>
        ...
        <Card>
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700">Vista de clientes</div>
              <h2 className="mt-2 text-xl font-black text-stone-900">Clientes</h2>
            </div>
            <Input placeholder="Buscar por nombre, teléfono, email, empresa..." value={query} onChange={(event) => setQuery(event.target.value)} />
          </div>
          ...
          <div className="mb-4 grid gap-3 md:grid-cols-5">
            {topCustomers.map(({ customer, stats }) => (
              <button key={customer.id} type="button" onClick={() => selectCustomerRecord(customer)} className="rounded-2xl border border-stone-200 bg-stone-50 p-4 text-left transition hover:border-emerald-300">
                ...
                <div className="mt-1 text-xs text-stone-500">{stats.purchaseCount} venta(s)</div>
          ...
          <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {!customer.isActive ? <Badge tone="red">Archivado</Badge> : null}
                      </div>
                      <div className="mt-1 text-sm text-stone-500">{customer.phone || 'Sin teléfono'}{customer.email ? ` / ${customer.email}` : ''}</div>
                    </div>
                    <div className="grid grid-cols-2 gap-3 text-sm lg:min-w-[280px]">
                      <div><div className="text-xs uppercase tracking-[0.14em] text-stone-400">Gasto total</div><div className="font-semibold text-stone-900">{money(stats.totalSpend, currencySymbol)}</div></div>
                      <div><div className="text-xs uppercase tracking-[0.14em] text-stone-400">Puntos</div><div className="font-semibold text-stone-900">{stats.loyaltyBalance}</div></div>
                      <div><div className="text-xs uppercase tracking-[0.14em] text-stone-400">Última compra</div><div className="font-semibold text-stone-900">{stats.lastPurchaseAt ? shortDate(stats.lastPurchaseAt) : 'Sin ventas aún'}</div></div>
                      <div><div className="text-xs uppercase tracking-[0.14em] text-stone-400">Por cobrar</div><div className="font-semibold text-stone-900">{money(stats.receivableBalance, currencySymbol)}</div></div>
                    </div>
                  </div>
                </button>
              );
            ...
            {!filteredCustomers.length ? <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-6 text-sm text-stone-500">No se encontraron clientes que coincidan con la búsqueda.</div> : null}
          </div>
        </Card>
      </div>
      ...
      {selectedCustomer ? (
        <Card>
          ...
          <div className="grid gap-6 xl:grid-cols-3">
            <div>
              <div className="mb-3 text-sm font-semibold text-stone-900">Ventas recientes</div>
              <div className="space-y-3">
                  ...
                  <div key={sale.id} className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="text-right">
                        ...
                        {sale.isCreditSale ? <Badge tone="amber">Crédito</Badge> : null}
                      </div>
                    </div>
                  </div>
                )) : <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-6 text-sm text-stone-500">No hay ventas completadas aún.</div>}
              </div>
            </div>

            <div>
              <div className="mb-3 text-sm font-semibold text-stone-900">Historial de fidelidad</div>
              <div className="space-y-3">
                  ...
                  <div key={entry.id} className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-stone-500">Saldo {entry.balanceAfter}</div>
                      </div>
                    </div>
                    {entry.note ? <div className="mt-2 text-sm text-stone-600">{entry.note}</div> : null}
                  </div>
                )) : <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-6 text-sm text-stone-500">No hay actividad de fidelidad registrada.</div>}
              </div>
            </div>

            <div>
              <div className="mb-3 text-sm font-semibold text-stone-900">Cuentas por cobrar</div>
              <div className="space-y-3">
                    ...
                    <div key={ledger.id} className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="text-xs text-stone-500">Vence {shortDate(ledger.dueDate)}</div>
                        </div>
                      ...
                      <div className="mt-1 flex justify-between text-sm"><span>Saldo</span><span className="font-semibold text-stone-900">{money(ledger.balance, currencySymbol)}</span></div>
                        ...
                        {ledger.payments.map((payment) => (
                          <div key={payment.id} className="rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-700">
                            <div className="flex justify-between gap-3">
                              ...
                              <span>{money(payment.amount, currencySymbol)}</span>
                            </div>
                            <div className="mt-1 text-xs text-stone-500">{shortDate(payment.paidAt)} por {payment.createdByUser.name ?? payment.createdByUser.email}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  );
                }) : <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 px-4 py-6 text-sm text-stone-500">No hay cuentas por cobrar registradas para este cliente.</div>}
              </div>

              {selectedCustomer.creditLedgers.some((ledger) => Number(ledger.balance) > 0) ? (
                <div className="mt-4 rounded-2xl border border-stone-200 bg-white p-4">
                  <div className="text-sm font-semibold text-stone-900">Registrar pago de cuenta por cobrar</div>
                  <div className="mt-3 grid gap-3">
                    <select className="rounded-xl border border-stone-300 bg-stone-50 px-4 py-2.5 text-sm" value={paymentDraft.customerCreditLedgerId} onChange={(event) => {
                      ...
                      {selectedCustomer.creditLedgers.filter((ledger) => Number(ledger.balance) > 0).map((ledger) => (
                        <option key={ledger.id} value={ledger.id}>{ledger.sale?.saleNumber ?? 'Venta a crédito'} / {money(ledger.balance, currencySymbol)}</option>
                      ))}
                    </select>
                    ...
                    <Input placeholder="Número de referencia" value={paymentDraft.referenceNumber} onChange={(event) => setPaymentDraft((current) => ({ ...current, referenceNumber: event.target.value }))} />
                  ...
                  <div className="mt-4">
                    <Button type="button" onClick={() => void postPayment()} disabled={postingPayment}>
                      {postingPayment ? 'Registrando...' : 'Registrar pago'}
                    </Button>
                  </div>
                </div>
              ) : null}
