...
export default function StaffListManager({
  ...
  async function toggleActive(item: SerializedStaffListItem) {
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo actualizar el estado del personal.' }));
    setLoadingId(null);

    if (!response.ok || !data?.item) {
      setError(data?.error ?? 'No se pudo actualizar el estado del personal.');
      return;
    }

    setItems((current) => current.map((entry) => (entry.id === item.id ? data.item : entry)));
    setSuccess(
      `${data.item.name} ahora está ${data.item.isActive ? 'activo' : 'inactivo'} en ${data.item.shopName}.`
    );
  }
    ...
    <div className="space-y-6">
      <Card>
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-emerald-700">Operaciones de personal</div>
            <h2 className="mt-2 text-2xl font-black text-stone-950">Acceso del equipo de un vistazo</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-500">
              Busca personal, revisa roles y asignaciones de tienda, y activa o desactiva accesos sin salir del espacio de trabajo.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="rounded-[22px] border border-stone-200 bg-stone-50 px-4 py-3">
              <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Inactivos</div>
              <div className="mt-1 text-2xl font-black text-stone-950">{inactiveCount}</div>
            </div>
            <div className="rounded-[22px] border border-stone-200 bg-stone-50 px-4 py-3">
              <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Vistos esta semana</div>
        ...
        <div className="mt-6 grid gap-3 lg:grid-cols-[minmax(0,1.6fr)_repeat(3,minmax(0,1fr))]">
          <Input
            ...
            placeholder="Buscar nombre, correo, rol o tienda..."
          />
          ...
          <select className={selectClassName} value={shopFilter} onChange={(event) => setShopFilter(event.target.value)}>
            <option value="">Todas las tiendas gestionadas</option>
            {shops.map((shop) => (
      ...
      <Card>
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-400">Directorio de personal</div>
            <h2 className="mt-2 text-xl font-black text-stone-950">Asignaciones y accesos</h2>
            <p className="mt-1 text-sm text-stone-500">
              {filteredItems.length} registro(s) coinciden con los filtros actuales.
            </p>
          </div>

          <Link href="/staff/new">
            <Button>Añadir cuenta de personal</Button>
          </Link>
        </div>
        ...
        <div className="overflow-hidden rounded-[26px] border border-stone-200">
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-stone-50 text-stone-500">
                <tr>
                  <th className="px-4 py-3.5">Personal</th>
                  <th className="px-4 py-3.5">Rol</th>
                  <th className="px-4 py-3.5">Tienda</th>
                ...
                {filteredItems.map((item) => (
                  <tr key={item.id} className="border-t border-stone-200 bg-white transition hover:bg-stone-50/70">
                    ...
                    <td className="px-4 py-4 text-stone-600">
                      {item.lastLogin ? dateTime(item.lastLogin) : 'Nunca'}
                    </td>
                    <td className="px-4 py-4">
          ...
          {!filteredItems.length ? (
            <div className="border-t border-stone-200 bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              Ningún miembro del personal coincide con la búsqueda. Ajusta los filtros o crea una cuenta nueva.
            </div>
          ) : null}
        </div>
      </Card>
