    ...
    links: [
      {
        href: '/dashboard',
        label: 'Panel',
        description: 'Estado del negocio y puntos de atención en vivo.',
        icon: 'dashboard',
        minRole: 'CASHIER'
      },
      {
        href: '/sales',
        label: 'Ventas',
        description: 'Revisa transacciones completadas y recibos.',
        icon: 'sales',
        minRole: 'CASHIER'
      },
      {
        href: '/returns',
        label: 'Devoluciones',
        description: 'Controla anulaciones, reembolsos, cambios y aprobaciones.',
        icon: 'returns',
        minRole: 'CASHIER'
      },
      {
        ...
        description: 'Realiza conteos formales y registra variaciones aprobadas.',
        icon: 'stock-counts',
        minRole: 'CASHIER'
      }
    ]
    ...
    links: [
      {
        ...
        description: 'Inicia una venta y emite recibos rápidamente.',
        icon: 'checkout',
        minRole: 'CASHIER'
      },
      {
        ...
        description: 'Revisa carritos retenidos y cotizaciones imprimibles.',
        icon: 'sales',
        minRole: 'CASHIER'
      },
      {
        href: '/register/open',
        label: 'Abrir caja',
        description: 'Inicia una sesión con el fondo inicial de efectivo.',
        icon: 'register',
        minRole: 'CASHIER'
      },
      {
        href: '/register/close',
        label: 'Cerrar caja',
        description: 'Cuenta el efectivo, compara totales y cierra cajas activas.',
        icon: 'register',
        minRole: 'CASHIER'
      },
      {
        ...
        description: 'Revisa sesiones anteriores y variaciones.',
        icon: 'register',
        minRole: 'CASHIER'
      },
      {
        href: '/inventory',
        label: 'Inventario',
        description: 'Controla niveles y ajustes de inventario.',
        icon: 'inventory',
        minRole: 'MANAGER',
      ...
      },
      {
        ...
        description: 'Mueve inventario entre sucursales y controla envíos y recepciones.',
        icon: 'transfers',
        minRole: 'MANAGER'
      },
      {
        href: '/purchases',
        label: 'Compras',
        description: 'Gestiona pedidos a proveedores y recepciones.',
        icon: 'purchases',
        minRole: 'MANAGER'
      },
      {
        href: '/activity',
        label: 'Actividad',
        description: 'Revisa el registro de auditoría operativa.',
        icon: 'activity',
        minRole: 'MANAGER'
      },
      {
        href: '/staff',
        label: 'Personal',
        description: 'Gestiona empleados, roles y accesos.',
        icon: 'staff',
        minRole: 'ADMIN',
    ...
    links: [
      {
        href: '/products',
        label: 'Productos',
        description: 'Gestiona artículos vendibles y precios.',
        icon: 'products',
        minRole: 'MANAGER'
      },
      {
        href: '/categories',
        label: 'Categorías',
        description: 'Mantén el catálogo organizado.',
        icon: 'categories',
        minRole: 'MANAGER'
      },
      {
        href: '/customers',
        label: 'Clientes',
        description: 'Controla historial, fidelización y cuentas por cobrar.',
        icon: 'customers',
        minRole: 'MANAGER'
      },
      {
        href: '/suppliers',
        label: 'Proveedores',
        description: 'Mantén relaciones y datos de proveedores.',
        icon: 'suppliers',
        minRole: 'MANAGER'
      }
    ]
    ...
    links: [
      {
        href: '/reports',
        label: 'Reportes',
        description: 'Reportes de ventas, inventario, ganancias y control de caja.',
        icon: 'reports',
        minRole: 'ADMIN',
      ...
      },
      {
        href: '/settings',
        label: 'Configuración',
        description: 'Reglas, impuestos, recibos y valores predeterminados.',
        icon: 'settings',
        minRole: 'MANAGER'
      }
    ]
...
export default function AppSidebar({
  ...
  async function switchShop(nextShopId: string) {
    ...
    const data = await response.json().catch(() => ({ error: 'No se puede cambiar de sucursal en este momento.' }));

    if (!response.ok) {
      setSwitchError(data.error ?? 'No se puede cambiar de sucursal en este momento.');
    ...
    <div className="flex h-full flex-col gap-4 p-4">
      <div className="rounded-[32px] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(255,255,255,0.84))] p-4 shadow-[0_24px_48px_-30px_rgba(28,25,23,0.36)] backdrop-blur">
        <div className="flex items-start justify-between gap-3">
          <div className={`min-w-0 ${isCollapsed ? 'w-full text-center' : ''}`}>
            ...
            {!isCollapsed ? (
              <>
                <div className="mt-4 text-[11px] font-semibold uppercase tracking-[0.24em] text-emerald-700">Punto de venta</div>
                <div className="mt-1 truncate text-xl font-black text-stone-950">{shopName}</div>
                <div className="mt-1 text-sm text-stone-500">Gestión de cajas, inventario y operaciones.</div>
                ...
                {availableShops.length > 1 ? (
                  <div className="mt-4 space-y-2">
                    <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-400">Sucursal activa</div>
                    <select
      ...
      {!isCollapsed ? (
        <div className="rounded-[30px] border border-stone-200/80 bg-white/82 p-4">
          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-stone-400">Linea rápida</div>
      ...
      <div className="border-b border-white/70 bg-white/78 px-4 py-3 backdrop-blur lg:hidden">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-emerald-700">Sistema POS</div>
