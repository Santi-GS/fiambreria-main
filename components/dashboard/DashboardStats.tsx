...
const items = [
  {
    ...
    helper: 'Artículos vendibles del catálogo listos para cobrar.',
    meta: (stockCoverage: number) => `${stockCoverage}% sobre el nivel de reposición`,
    icon: 'products',
    tone: {
  ...
  {
    label: 'Productos con existencias bajas',
    helper: 'Artículos que requieren atención de reposición.',
    meta: (stockCoverage: number, lowStockCount: number) =>
      lowStockCount === 0 ? 'La cola de reposición está despejada' : `${100 - stockCoverage}% del catálogo requiere revisión`,
    icon: 'alert',
    tone: {
  ...
  },
  {
    ...
    helper: 'Órdenes de compra abiertas en proceso de adquisición.',
    meta: (_stockCoverage: number, _lowStockCount: number, pendingPurchases: number) =>
      pendingPurchases === 0 ? 'La cola de recepción está despejada' : `${pendingPurchases} compra(s) abierta(s)`,
    icon: 'purchases',
    tone: {
...
export default function DashboardStats({
  ...
  const visibleItems = showPendingPurchases ? items : items.filter((item) => item.label !== 'Compras pendientes');
