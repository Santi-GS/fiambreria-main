  ...
  return (
    <Card className="overflow-hidden border-stone-200/80 bg-white/95">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-amber-700">Cola de reposición</div>
          <h2 className="mt-2 text-2xl font-black text-stone-900">Vigilancia de existencias bajas</h2>
          <p className="mt-1 text-sm text-stone-500">Productos que se acercan o ya están por debajo de su nivel de reposición.</p>
        </div>
        <Link
          href="/inventory"
      ...
      <div className="space-y-3">
        {products.length ? (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-sm text-stone-500">{product.category?.name ?? 'Sin categoría'}</div>
                    </div>
                    <Badge tone={isCritical ? 'red' : 'amber'}>{isCritical ? 'Crítico' : 'Vigilar'}</Badge>
                  </div>
                  ...
                  <div className="mt-4 rounded-[22px] bg-white/86 p-3">
                    <div className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">
                      <span>Disponible {product.stockQty}</span>
                      <span>Nivel de reposición {product.reorderPoint}</span>
                    </div>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3 text-sm text-stone-600">
                    <span>Quedan {product.stockQty} unidad(es) antes de reponer.</span>
          ...
          <div className="rounded-[28px] border border-dashed border-stone-300 bg-stone-50 p-5">
            <div className="text-sm font-semibold text-stone-900">No hay productos con existencias bajas.</div>
            <div className="mt-2 text-sm text-stone-500">El inventario de la sucursal está saludable. Mantén actualizadas las recepciones y los ajustes para que las alertas sigan siendo útiles.</div>
