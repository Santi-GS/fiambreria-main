...
export default function ReportFilters({
  ...
  return (
    <form action={action} method="GET" className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
      <label className="space-y-1 text-sm">
        <div className="font-medium text-stone-600">Desde</div>
        <Input type="date" name="from" defaultValue={fromValue} />
      </label>
      <label className="space-y-1 text-sm">
        <div className="font-medium text-stone-600">Hasta</div>
        <Input type="date" name="to" defaultValue={toValue} />
      </label>

      {cashiers.length ? (
        <SelectField name="cashierId" value={cashierId} label="Cajero" options={cashiers} />
      ) : null}

      {categories.length ? (
        <SelectField name="categoryId" value={categoryId} label="Categoría" options={categories} />
      ) : null}

      {paymentMethods.length ? (
        <SelectField name="paymentMethod" value={paymentMethod} label="Pago" options={paymentMethods} />
      ) : null}

      {products.length ? (
        <SelectField name="productId" value={productId} label="Producto" options={products} />
      ) : null}
