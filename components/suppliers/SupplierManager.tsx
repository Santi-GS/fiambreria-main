...
export default function SupplierManager({
  ...
  async function saveSupplier(event: React.FormEvent<HTMLFormElement>) {
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo guardar el proveedor.' }));
    if (!response.ok) {
  ...
  async function toggleSupplier(supplier: Supplier) {
    const confirmed = window.confirm(`${supplier.isActive ? '¿Archivar' : '¿Restaurar'} ${supplier.name}?`);
    if (!confirmed) {
      return;
    }
    ...
    if (!response.ok || !data?.supplier) {
      setError(data?.error ?? 'No se pudo actualizar el proveedor.');
      return;
    }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
      <Card>
        <h2 id="new-supplier" className="text-xl font-black text-stone-900">Añadir proveedor</h2>
        <form onSubmit={saveSupplier} className="mt-5 grid gap-4">
          <Input placeholder="Nombre del proveedor" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} required />
          <Input placeholder="Nombre de contacto" value={form.contactName} onChange={(event) => setForm((current) => ({ ...current, contactName: event.target.value }))} />
      ...
      <Card>
        <h2 className="text-xl font-black text-stone-900">Lista de proveedores</h2>
              ...
              <div key={supplier.id} className="rounded-2xl border border-stone-200 p-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex flex-wrap gap-2">
                      <Badge tone={supplier.isActive ? 'emerald' : 'stone'}>
                        {supplier.isActive ? 'Activo' : 'Archivado'}
                      </Badge>
                      <Badge tone="blue">{supplier._count?.purchases ?? 0} compra(s)</Badge>
                      <Badge tone="amber">{supplier._count?.invoices ?? 0} factura(s)</Badge>
                      {supplier.overdueCount > 0 ? (
                        <Badge tone="red">{supplier.overdueCount} vencida(s)</Badge>
                      ) : null}
                    </div>
                    <div className="mt-2 text-sm text-stone-500">
                      {supplier.contactName || 'Sin contacto'} / {supplier.phone || 'Sin teléfono'}
                    </div>
                    <div className="text-xs text-stone-500">{supplier.email || 'Sin correo registrado'}</div>
                    ...
                    <div className="text-xs text-stone-500">{supplier.openInvoiceCount} factura(s) abierta(s)</div>
                    <div className="mt-3">
                      <Button type="button" variant="secondary" onClick={() => toggleSupplier(supplier)}>
                        {supplier.isActive ? 'Archivar' : 'Restaurar'}
                      </Button>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-4 xl:grid-cols-2">
                  <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                    <div className="text-sm font-semibold text-stone-900">Saldos pendientes</div>
                          ...
                          .map((invoice) => (
                            <div key={invoice.id} className="rounded-2xl border border-stone-200 bg-white px-4 py-3">
                              <div className="flex items-center justify-between gap-3">
                                <div>
                                  ...
                                  <div className="text-xs text-stone-500">
                                    {invoice.purchase.purchaseNumber} / Vence el {shortDate(invoice.dueDate)}
                                  </div>
                                </div>
                              ...
                              <div className="mt-2 text-sm text-stone-600">
                                Saldo {money(invoice.payableEntry?.balance ?? '0', currencySymbol)}
                              </div>
                            </div>
                          ))
                      ) : (
                        <div className="text-sm text-stone-500">No hay saldos pendientes de proveedores.</div>
                      )}
                    </div>
                  </div>

                  <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                    <div className="text-sm font-semibold text-stone-900">Pagos recientes</div>
                    <div className="mt-3 space-y-3">
                      {supplier.recentPayments.length ? (
                        ...
                        <div className="text-sm text-stone-500">Aún no hay pagos a proveedores registrados.</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {supplier.notes ? (
                  <div className="mt-4 text-sm text-stone-500">Notas: {supplier.notes}</div>
                ) : null}
              </div>
            ))
          ) : (
            <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-stone-500">
              Aún no hay proveedores. Añade uno para comenzar a registrar compras.
            </div>
          )}
        </div>
      </Card>
