...
export default function StockCountDetailManager({
  ...
  async function saveSheet(showSuccessMessage = true) {
    ...
    try {
      ...
      await applyResponse(response, 'No se pudo guardar el conteo de inventario.');
      if (showSuccessMessage) {
        setSuccess('Hoja de conteo guardada.');
      }
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'No se pudo guardar el conteo de inventario.');
      throw saveError;
    } finally {
  ...
  async function runTransition(action: 'SUBMIT' | 'APPROVE' | 'POST' | 'CANCEL') {
    ...
    try {
      ...
      await applyResponse(response, 'No se pudo actualizar el conteo de inventario.');
      setSuccess(
        action === 'SUBMIT'
          ? 'Conteo de inventario enviado para aprobación.'
          : action === 'APPROVE'
            ? 'Conteo de inventario aprobado.'
            : action === 'POST'
              ? 'Variaciones aplicadas al inventario.'
              : 'Conteo de inventario cancelado.'
      );
    } catch (transitionError) {
      setError(transitionError instanceof Error ? transitionError.message : 'No se pudo actualizar el conteo de inventario.');
    } finally {
      setLoadingAction(null);
    }
  }
      ...
      <div className="grid gap-6 xl:grid-cols-[0.96fr_1.04fr]">
        <Card className="space-y-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-700">Conteo de inventario</div>
              <h2 className="mt-2 text-2xl font-black text-stone-950">{stockCount.referenceNumber}</h2>
              <p className="mt-2 text-sm text-stone-500">
                {stockCount.title ?? 'Conteo de inventario sin título'}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Badge tone={toneForStatus(stockCount.status)}>{getStockCountStatusLabel(stockCount.status)}</Badge>
              {stockCount.isBlind ? <Badge tone="stone">Conteo ciego</Badge> : null}
            </div>
          </div>

          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[24px] border border-stone-200 bg-stone-50 px-4 py-4">
              <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Progreso</div>
              <div className="mt-2 text-3xl font-black text-stone-950">
                {draftCountedItemCount} / {stockCount.itemCount}
              </div>
              <div className="mt-1 text-sm text-stone-500">Productos contados</div>
            </div>
            <div className="rounded-[24px] border border-stone-200 bg-stone-50 px-4 py-4">
              <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Líneas con variación</div>
              <div className="mt-2 text-3xl font-black text-stone-950">
                {stockCount.revealBlindQuantities ? draftVarianceLineCount : 'Oculto'}
              </div>
              <div className="mt-1 text-sm text-stone-500">
                {stockCount.revealBlindQuantities ? 'Líneas actualmente distintas de lo esperado' : 'El conteo ciego aún no está conciliado'}
              </div>
            </div>
          </div>

          <div className="rounded-[24px] border border-stone-200 bg-stone-50 px-4 py-4 text-sm text-stone-600">
            <div>Creado por <span className="font-semibold text-stone-900">{stockCount.createdByUser.name ?? stockCount.createdByUser.email}</span> el <span className="font-semibold text-stone-900">{dateTime(stockCount.createdAt)}</span></div>
            <div className="mt-2">
              {stockCount.approvedByUser
                ? <>Aprobado por <span className="font-semibold text-stone-900">{stockCount.approvedByUser.name ?? stockCount.approvedByUser.email}</span> el <span className="font-semibold text-stone-900">{stockCount.approvedAt ? dateTime(stockCount.approvedAt) : 'No disponible'}</span></>
                : 'La aprobación aún no se ha completado.'}
            </div>
            <div className="mt-2">
              {stockCount.postedAt
                ? <>Aplicado al inventario el <span className="font-semibold text-stone-900">{dateTime(stockCount.postedAt)}</span></>
                : stockCount.submittedAt
                  ? <>Enviado el <span className="font-semibold text-stone-900">{dateTime(stockCount.submittedAt)}</span></>
                  : stockCount.startedAt
                    ? <>Iniciado el <span className="font-semibold text-stone-900">{dateTime(stockCount.startedAt)}</span></>
                    : 'El conteo aún no ha comenzado.'}
            </div>
          </div>

          {stockCount.isBlind && !stockCount.revealBlindQuantities ? (
            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-900">
              El modo de conteo ciego está activo. Las cantidades esperadas y las variaciones permanecerán ocultas hasta enviar el conteo para conciliación.
            </div>
          ) : null}

          <div className="space-y-3">
            <label className="text-sm font-semibold text-stone-700">Notas del conteo</label>
            <textarea
              value={sheetNotes}
          ...
          <div className="flex flex-wrap gap-3">
            {editable ? (
              ...
              >
                {loadingAction === 'SAVE' ? 'Guardando...' : 'Guardar hoja'}
              </Button>
            ) : null}
            ...
            {canSubmit ? (
              ...
              >
                {loadingAction === 'SUBMIT' ? 'Enviando...' : 'Enviar para aprobación'}
              </Button>
            ) : null}
            ...
            {canApprove ? (
              ...
              >
                {loadingAction === 'APPROVE' ? 'Aprobando...' : 'Aprobar conteo'}
              </Button>
            ) : null}
            ...
            {canPost ? (
              ...
              >
                {loadingAction === 'POST' ? 'Aplicando...' : 'Aplicar variaciones'}
              </Button>
            ) : null}
            ...
            {canCancel ? (
              ...
              >
                {loadingAction === 'CANCEL' ? 'Cancelando...' : 'Cancelar conteo'}
              </Button>
            ) : null}

            <Link href="/stock-counts">
              <Button type="button" variant="ghost">Volver a los conteos</Button>
            </Link>
          </div>
        </Card>
        ...
        <Card className="space-y-5">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-stone-400">Hoja de conteo</div>
              <h2 className="mt-2 text-2xl font-black text-stone-950">Registro por producto</h2>
              <p className="mt-2 text-sm leading-6 text-stone-500">
                Cuenta cada producto una vez, añade notas cuando sea necesario y concilia únicamente mediante el flujo de aprobación y aplicación.
              </p>
            </div>
          ...
          <div className="overflow-hidden rounded-[26px] border border-stone-200">
              ...
              <table className="min-w-full text-left text-sm">
                <thead className="bg-stone-50 text-stone-500">
                  <tr>
                    ...
                    <th className="px-4 py-3.5">Esperado</th>
                    <th className="px-4 py-3.5">Real</th>
                    <th className="px-4 py-3.5">Variación</th>
                    <th className="px-4 py-3.5">Nota</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={item.id} className="border-t border-stone-200 bg-white align-top transition hover:bg-stone-50/70">
                        <td className="px-4 py-4">
                          ...
                          <div className="mt-1 text-xs text-stone-500">Creado el {shortDate(item.createdAt)}</div>
                        </td>
                        <td className="px-4 py-4 text-stone-600">{item.skuSnapshot ?? 'N/A'}</td>
                        <td className="px-4 py-4 text-stone-600">
                          {stockCount.revealBlindQuantities ? item.expectedQty : 'Oculto'}
                        </td>
                        <td className="px-4 py-4">
                          {editable ? (
                            <Input
                            ...
                            <div className="font-semibold text-stone-900">{actualQty ?? 'Sin contar'}</div>
                          )}
                        </td>
                        <td className="px-4 py-4">
                          <div className={`font-semibold ${stockCount.revealBlindQuantities ? varianceTone(varianceQty) : 'text-stone-500'}`}>
                            {stockCount.revealBlindQuantities ? formatVariance(varianceQty) : 'Oculto'}
                          </div>
                        </td>
                        <td className="min-w-[220px] px-4 py-4">
                          {editable ? (
                            <Input
                              ...
                              placeholder="Nota opcional del conteo"
                            />
                          ) : (
                            <div className="text-stone-600">{draftLines[item.id]?.note || 'Sin nota'}</div>
                          )}
                        </td>
                      </tr>
                    );
            ...
            {!filteredItems.length ? (
              <div className="border-t border-stone-200 bg-stone-50 py-8 text-center text-sm text-stone-500">
                Ninguna línea de conteo coincide con la búsqueda actual.
              </div>
            ) : null}
          </div>
        </Card>
