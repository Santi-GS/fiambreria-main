...
export default function RegisterHistoryTable({
  ...
  async function reviewSession(session: SerializedCashSession) {
    const reviewNote = window.prompt(
      'Nota opcional de revisión del gerente',
      session.reviewNote ?? ''
    );
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo revisar la sesión de caja.' }));
    setLoadingId(null);

    if (!response.ok || !data?.session) {
      setError(data?.error ?? 'No se pudo revisar la sesión de caja.');
      return;
    }
  ...
  async function reopenSession(session: SerializedCashSession) {
    const reason = window.prompt('Motivo para reabrir este turno cerrado');
    ...
    const data = await response.json().catch(() => ({ error: 'No se pudo reabrir la sesión de caja.' }));
    setLoadingId(null);

    if (!response.ok || !data?.session) {
      setError(data?.error ?? 'No se pudo reabrir la sesión de caja.');
      return;
    }
    ...
    <div className="space-y-6">
      <Card>
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-emerald-700">Historial de caja</div>
            <h2 className="mt-2 text-2xl font-black text-stone-950">Cronología de sesiones</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-500">
              Revisa aperturas y cierres anteriores, expectativas de efectivo, efectivo contado, aprobaciones, reaperturas y diferencias positivas o negativas.
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-[22px] border border-stone-200 bg-stone-50 px-4 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Sesiones</div>
              <div className="mt-1 text-2xl font-black text-stone-950">{sessions.length}</div>
            </div>
            <div className="rounded-[22px] border border-stone-200 bg-stone-50 px-4 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Abiertas</div>
              <div className="mt-1 text-2xl font-black text-sky-700">{openSessions}</div>
            </div>
            <div className="rounded-[22px] border border-stone-200 bg-stone-50 px-4 py-3">
                <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-400">Diferencia neta</div>
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="bg-stone-50 text-stone-500">
                <tr>
                  ...
                  <th className="px-4 py-3.5">Apertura</th>
                  <th className="px-4 py-3.5">Cierre</th>
                  <th className="px-4 py-3.5">Fondo</th>
                  <th className="px-4 py-3.5">Esperado</th>
                  <th className="px-4 py-3.5">Real</th>
                  <th className="px-4 py-3.5">Diferencia</th>
                ...
                {sessions.map((session) => (
                  <tr key={session.id} className="border-t border-stone-200 bg-white align-top">
                    <td className="px-4 py-4">
                      ...
                      <div className="mt-1 text-xs text-stone-500">{session.cashierEmail ?? 'Cuenta de cajero'}</div>
                      {session.reopenedAt ? (
                        <div className="mt-2 text-xs text-amber-700">
                          Reabierta {dateTime(session.reopenedAt)}
                          {session.reopenedByName ? ` por ${session.reopenedByName}` : ''}
                        </div>
                      ) : null}
                    </td>
                    ...
                    <td className="px-4 py-4 text-stone-600">
                      {session.closedAt ? (
                        <>
                          ...
                          {session.closedByName ? <div className="mt-1 text-xs text-stone-500">Por {session.closedByName}</div> : null}
                        </>
                      ) : 'Aún abierta'}
                    </td>
                    <td className="px-4 py-4 font-semibold text-stone-900">{money(session.openingFloat, currencySymbol)}</td>
                    <td className="px-4 py-4 text-stone-700">{session.closingExpected ? money(session.closingExpected, currencySymbol) : 'Esperando cierre'}</td>
                    <td className="px-4 py-4 text-stone-700">{session.closingActual ? money(session.closingActual, currencySymbol) : 'Esperando conteo'}</td>
                    <td className="px-4 py-4">
                      <Badge tone={varianceTone(session.variance)}>
                        {session.variance ? money(session.variance, currencySymbol) : 'Esperando cierre'}
                      </Badge>
                    </td>
                    <td className="px-4 py-4">
                      {session.reviewedAt ? (
                        <div className="text-xs text-stone-600">
                          <div className="font-semibold text-emerald-700">Aprobada</div>
                          <div>{dateTime(session.reviewedAt)}</div>
                          {session.reviewedByName ? <div>Por {session.reviewedByName}</div> : null}
                        </div>
                      ) : (
                        <span className="text-xs text-stone-500">Pendiente de revisión</span>
                      )}
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex min-w-[220px] flex-col gap-2">
                        ...
                        {canManageSessions && session.status !== 'OPEN' ? (
                          ...
                          >
                            {loadingId === session.id ? 'Guardando...' : session.reviewedAt ? 'Actualizar revisión' : 'Aprobar'}
                          </Button>
                        ) : null}
                        {canManageSessions && session.status !== 'OPEN' ? (
                          <Button
                        ...
                        {session.status === 'OPEN' ? (
                          <Link href="/register/close" className="inline-flex">
                            <Button type="button" className="w-full justify-center">Ir al cierre</Button>
                          </Link>
                        ) : null}
                      </div>
                    </td>
            ...
            {sessions
              ...
              .map((session) => (
                <div key={session.id} className="rounded-[24px] border border-stone-200 bg-stone-50 px-4 py-4">
                  ...
                  {session.reviewNote ? <div className="mt-3 rounded-[18px] border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">Revisión del gerente: {session.reviewNote}</div> : null}
                  {session.reopenReason ? <div className="mt-3 rounded-[18px] border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Motivo de reapertura: {session.reopenReason}</div> : null}
                </div>
              ))}
          </div>
        ) : null}
