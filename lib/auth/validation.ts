...
const imageUrlSchema = z
  ...
  .max(600000, 'La imagen es demasiado grande.')
  .refine(
    (value) => value.startsWith('data:image/') || /^https?:\/\//i.test(value),
    'Usa una imagen cargada o una URL de imagen válida.'
  );
...
const productVariantSchema = z.object({
  ...
  'Añade al menos un identificador de variante.'
);
...
export const loginSchema = z.object({
  email: z.string().trim().email('Introduce un correo válido').transform((value) => value.toLowerCase()),
...
export const productSchema = z.object({
  ...
  baseUnitOfMeasureId: z.string().trim().min(1, 'Selecciona una unidad base.'),
  sku: z.string().trim().max(50).optional().nullable(),
  barcode: z.string().trim().max(60).optional().nullable(),
  name: z.string().trim().min(2).max(120),
  ...
  uomConversions: z.array(z.object({
    ...
    ratioToBase: z.coerce.number().int().positive('La conversión del paquete debe ser mayor que cero.')
  })).default([]),
